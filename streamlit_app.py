...
# --- AKILLI İNTERNET ARAMA OPTİMİZASYONU ---
...
st.write(f"Şu anki mod: **{kisilik}**")
...
if soru_girdisi := st.chat_input("Mesajınızı buraya yazın..."):
    ...
    with st.chat_message("assistant"):
        ...
        # --- DİL GEÇİŞİ VE SES TALİMATI EKİ ---
        # Yapay zekaya eğer kullanıcı İngilizce yazdıysa modu otomatik değiştirmesini söylüyoruz
        sistem_talimati = (
            f"{karakter_talimati} Sağlanan dökümanları ve geçmişi dikkate alarak cevap üret. "
            "ÇOK ÖNEMLİ KURAL: Eğer kullanıcı sana İngilizce (English) yazarsa, "
            "seçilen kişilikten bağımsız olarak sen de ANINDA tamamen İngilizce cevap ver, "
            "onunla akıcı bir şekilde İngilizce konuş ve pratik yapmasına destek ol."
        )
        ...
        try:
            ...
            # --- AKILLI SESLENDİRME DİLİ SEÇİMİ ---
            # Cevap metninde en çok kullanılan kelimelere bakarak seslendirme dilini seçiyoruz
            # Eğer İngilizce kelimeler yoğunluktaysa gTTS robotu İngilizce aksanıyla konuşacak
            ingilizce_mi = any(w in yanit.lower() for w in ["the", "you", "are", "is", "good", "hello", "hi", "welcome", "i", "am"])
            ses_dili = 'en' if ingilizce_mi else 'tr'
            ses_tld = 'com' if ingilizce_mi else 'com.tr'
            
            ses_metni = re.sub(r'[^\w\s,.!?:\(\)\-\"\']', '', yanit)
            with st.spinner("🔊 Ses dosyası hazırlanıyor..."):
                tts = gTTS(text=ses_metni[:300], lang=ses_dili, tld=ses_tld)
                tts.save("cevap.mp3")
